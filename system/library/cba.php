...
class CBA {
    ...
    public function processFeedResponses($settings, $db) {
        $qry = $db->query("SELECT `submission_id` FROM `order_amazon_report` WHERE `status` = 'processing'");
        ...
        for ($i = 0; $i < count($submission_ids); $i += 50) {
            ...
            if (isset($xml->GetReportListResult) || isset($xml->GetReportListByNextTokenResult)) {
                ...
                foreach ($list as $list_item) {
                    ...
                    $db->query("UPDATE `order_amazon_report` SET `status` = '" . $db->escape($status) . "', text = '" . $db->escape($error_message) . "' WHERE `submission_id` = '" . $db->escape($submission_id) . "'");
        ...
        while ($xml && (isset($xml->GetReportListResult) || isset($xml->GetReportListByNextTokenResult))) {
            ...
            foreach ($list as $list_item) {
                ...
                foreach ($report_xml->Message as $message) {
                    ...
                    $result = $db->query("SELECT `order_id` FROM `order_amazon` WHERE `amazon_order_id` = '" . $db->escape($amazon_order_id) . "'")->row;
                    ...
                    $db->query("UPDATE `order` AS `o`, `order_amazon` `oa` SET `o`.`payment_firstname` = '" . $db->escape($billing_name) . "', `o`.`firstname` = '" . $db->escape($billing_name) . "', `o`.`email` = '" . $db->escape($billing_email) . "', `o`.`telephone` = '" . $db->escape($billing_phone_number) . "', `o`.`shipping_firstname` = '" . $db->escape($shipping_name) . "', `o`.`shipping_address_1` = '" . $db->escape($shipping_address1) . "', `o`.`shipping_address_2` = '" . $db->escape($shipping_address2) . "', `o`.`shipping_city` = '" . $db->escape($shipping_city) . "', `o`.`shipping_zone` = '" . $db->escape($shipping_zone) . "', `o`.`shipping_country` = '" . $db->escape($shipping_country_code) . "', `o`.`shipping_postcode` = '" . $db->escape($shipping_post_code) . "', `o`.`order_status_id` = " . (int) $settings->get('amazon_checkout_order_ready_status') . " WHERE `o`.`order_id` = " . (int) $order_id);

                    $db->query("INSERT INTO `order_history` (`order_id`, `order_status_id`, `comment`, `date_added`) VALUES (" . (int) $order_id . ", " . (int) $settings->get('amazon_checkout_order_ready_status') . ", '', NOW())");
                    ...
                    foreach ($message->OrderReport->Item as $item) {
                        ...
                        $db->query("REPLACE INTO `order_amazon_product` (`order_product_id`, `amazon_order_item_code`) SELECT `op`.`order_product_id`, '" . $db->escape($amazon_order_item_code) . "' FROM `order_product` `op` WHERE `op`.`order_product_id` = '" . $db->escape($order_product_id) . "'");
                    }
                }
