<?php
class ControllerTasksStartTask extends Controller
{
  ...
  public function index()
  {
    ...
    else
    {
      ...
      // Load messages
      $this->messages->Load( $this->data, 'tasks', 'start_task', 'index', $this->language->Get_Language_Code() );
      ...
      $this->data['tasks_start_task_button_href'] = $this->url->link('tasks/start_task/Start_Task', '', 'SSL');
      $this->data['tasks_start_task_cancel_button_href'] =  $this->url->link(substr($this->session->Get('request_referer'), strpos($this->session->Get('request_prereferer'), '=')+1) , '', 'SSL');
  ...
  public function Start_Task()
  {
    ...
    // Load messages
    $this->messages->Load( $this->data, 'tasks', 'start_task', 'Start_Task', $this->language->Get_Language_Code() );
    ...
    $request_data_valid = true;
     ...
     //------------------------------------------------------------------------
      ...
      if( ! $this->request->Is_POST_Parameter_Exists( 'task_id' ) )
      {
        ...
        $this->error[ 'task_id' ] = $this->data[ 'tasks_start_task_task_id_error' ];
      ...
      if( ! $this->request->Is_POST_Parameter_Exists( 'message' ) )
      {
        ...
        // Set error message text
        $this->error[ 'message' ] = $this->data[ 'tasks_start_task_message_error' ];
     ...
     else
     {
        // Load data model
        $this->load->model('tasks/tasks');
        if(($this->model_tasks_tasks->Get_Status($data[ 'task_id' ])==='wait_for_processing')||($this->model_tasks_tasks->Get_Status($data[ 'task_id' ])==='wait_for_fixing')){
          $json['return_code'] = $this->model_tasks_tasks->Change_Status( $data[ 'task_id' ] , 'processing' );
          if($json['return_code'] && ($data[ 'message' ] != '')){
            $this->model_tasks_tasks->Add_Status_Message( $data[ 'task_id' ],  $data[ 'message' ], 'processing' );
          }
