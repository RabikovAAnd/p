<?php
class ControllerTasksReopenTask extends Controller
{
  ...
  public function index()
  {
    ...
    else
    {
      ...
      //------------------------------------------------------------------------
      // Load messages
      $this->messages->Load( $this->data, 'tasks', 'reopen_task', 'index', $this->language->Get_Language_Code() );
      ...
      $this->data['tasks_reopen_task_button_href'] = $this->url->link('tasks/reopen_task/Reopen_Task', '', 'SSL');
      $this->data['tasks_reopen_task_cancel_button_href'] =  $this->url->link(substr($this->session->Get('request_referer'), strpos($this->session->Get('request_prereferer'), '=')+1) , '', 'SSL');
  ...
  public function Reopen_Task()
  {
    ...
    // Load messages
    $this->messages->Load( $this->data, 'tasks', 'reopen_task', 'Reopen_Task', $this->language->Get_Language_Code() );
    ...
    $request_data_valid = true;
     ...
     //------------------------------------------------------------------------
      ...
      if( ! $this->request->Is_POST_Parameter_Exists( 'task_id' ) )
      {
        ...
        $this->error[ 'task_id' ] = $this->data[ 'tasks_reopen_task_task_id_error' ];
      ...
      if( ! $this->request->Is_POST_Parameter_Exists( 'message' ) )
      {
        ...
        // Set error message text
        $this->error[ 'message' ] = $this->data[ 'tasks_reopen_task_message_error' ];
     ...
     else
     {
        ...
        {
          $json['return_code'] = $this->model_tasks_tasks->Change_Status( $data[ 'task_id' ] , 'new' );
