<?php
class ControllerTasksRejectTask extends Controller
{
  ...
  public function index()
  {
    ...
    else
    {
      ...
      // Load messages
      $this->messages->Load( $this->data, 'tasks', 'reject_task', 'index', $this->language->Get_Language_Code() );
      ...
      $this->data['tasks_reject_task_button_href'] = $this->url->link('tasks/reject_task/Reject_Task', '', 'SSL');
      $this->data['tasks_reject_task_cancel_button_href'] =  $this->url->link(substr($this->session->Get('request_referer'), strpos($this->session->Get('request_prereferer'), '=')+1) , '', 'SSL');
  ...
  public function Reject_Task()
  {
    ...
    // Load messages
    $this->messages->Load( $this->data, 'tasks', 'reject_task', 'Reject_Task', $this->language->Get_Language_Code() );
    ...
    $request_data_valid = true;
     ...
     else
     {
        // Load data model
        $this->load->model('tasks/tasks');
        if($this->model_tasks_tasks->Get_Status($data[ 'task_id' ])==='wait_for_processing'){
